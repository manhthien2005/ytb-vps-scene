...
2. **Write the spec.** Save to `.collab/specs/<task-name>.md` containing:
   ...
   - **Interfaces:** Type hints, function signatures, expected behavior
...
**ALWAYS run the test suite (`pytest`) yourself after Codex builds.**
...
5. **Fix.** Implement based on the winning hypothesis. Run `pytest`.
