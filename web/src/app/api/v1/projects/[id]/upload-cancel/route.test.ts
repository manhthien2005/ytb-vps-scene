...
function postRequest(input: Readonly<{ origin?: string; body?: string; contentLength?: string }> = {}) {
  ...
  return new NextRequest(`http://localhost:3000/api/v1/projects/${PROJECT_ID}/upload-cancel`, {
    method: "POST",
    headers,
...
describe("/api/v1/projects/[id]/upload-cancel", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    setEnv();
  ...
  it("returns exact idempotent cancellation responses", async () => {
    ...
    expect(service.cancel).toHaveBeenLastCalledWith({
  ...
  it("returns only stable no-store error bodies", async () => {
    service.cancel.mockRejectedValueOnce(Object.assign(
    ...
    service.cancel.mockRejectedValueOnce(new Error("private internal detail"));
