---
name: ui-ux-pro-max
description: "Use when designing, building, reviewing, or improving UI/UX for web, mobile, or desktop interfaces, especially when choosing styles, palettes, typography, charts, layout, accessibility, motion, landing patterns, or stack-specific implementation guidance."
---
...
Resolve the bundled search script once from the project root, then reuse that absolute path in every command:

```powershell
$uiuxSearch = (Resolve-Path ".agents/skills/ui-ux-pro-max/scripts/search.py").Path
```

If the skill is installed somewhere else, resolve `scripts/search.py` relative to this `SKILL.md` instead. Do not assume a global `skills/` directory.
...
- **Stack**: infer it from the repository and user request; ask only when the implementation choice materially changes
...
```bash
python $uiuxSearch "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python $uiuxSearch "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python $uiuxSearch "<query>" --design-system --persist -p "Project Name"
```
...
```bash
python $uiuxSearch "<query>" --design-system --persist -p "Project Name" --page "dashboard"
```
...
```bash
python $uiuxSearch "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python $uiuxSearch "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
```bash
python $uiuxSearch "<keyword>" --domain <domain> [-n <max_results>]
```
...
| Icon selection | `icons` | `--domain icons "navigation settings"` |
| Google Fonts catalog | `google-fonts` | `--domain google-fonts "Vietnamese sans editorial"` |
...
```bash
python $uiuxSearch "<keyword>" --stack <stack>
```
...
| `icons` | Icon semantics and library guidance | navigation, settings, commerce, status |
| `google-fonts` | Searchable font catalog | Vietnamese, serif, geometric, editorial |

### Available Stacks

| Stack | Focus |
|-------|-------|
Run `python $uiuxSearch --help` for the authoritative stack list. It includes React, Next.js, Vue, Svelte, Astro, Angular, Laravel, Nuxt, Tailwind/shadcn, React Native, SwiftUI, Flutter, Jetpack Compose, Three.js, JavaFX, WPF, WinUI, UWP, Avalonia, and Uno Platform.

**JavaFX enterprise examples:**

```bash
python $uiuxSearch "atlantafx primer enterprise theme" --stack javafx
python $uiuxSearch "enterprise tableview density permission" --stack javafx
```
...
```bash
python $uiuxSearch "AI search tool modern minimal" --design-system -p "AI Search"
```
...
python $uiuxSearch "minimalism dark mode" --domain style

# Get UX best practices for search interaction and loading
python $uiuxSearch "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
python $uiuxSearch "rendering accessibility search state" --stack react
```
...
python $uiuxSearch "fintech crypto" --design-system

# Markdown - best for documentation
python $uiuxSearch "fintech crypto" --design-system -f markdown
```
...
- 默认图标库使用 **Phosphor (`@phosphor-icons/react`)**。本 skill 的 `data/icons.csv` 中列出的只是常用推荐图标，不是完整集合。
- 当推荐表中找不到合适的图标时：
